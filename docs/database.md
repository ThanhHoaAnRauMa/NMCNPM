...
| `directKey` | String | No | Lookup key for direct conversations: normalized mode plus sorted pair of user ids |
...
| `{ directKey: 1 }` sparse | Fast reuse lookup for one direct conversation per user pair and mode |
