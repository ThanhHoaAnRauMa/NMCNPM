...
describe('integrated feature API', () => {
  ...
  test('creates a direct conversation and returns it only to members', async () => {
    ...
    assert.ok(restoredForBob.body.conversations[0].clearedAt)
    const bobRestoredHistory = await request(app)
      .get(`/chat/${created.body.conversationId}/messages`)
      .set('Authorization', `Bearer ${bob.accessToken}`)
    assert.equal(bobRestoredHistory.status, 200)
    assert.deepEqual(bobRestoredHistory.body.messages.map((message) => message.encryptedContent), ['new-ciphertext'])
    const bobForensicHistory = await request(app)
      .get(`/chat/${created.body.conversationId}/messages?includeHidden=true`)
      .set('Authorization', `Bearer ${bob.accessToken}`)
    assert.equal(bobForensicHistory.body.messages.length, 2)
